...
This parameter is used to enable/disable BlueTooth 4.2 LE Secure Connections.;;help_end;;
IDC_STATIC_LE_SECURE_CONNECTIONS
Link Manager Version
This parameter is used to choose Link Manager Version between BT4.0 and BT4.2 .;;help_end;;
...
This parameter is used to choose the indication type of RF active. It is activated when RF_ACTIVED_IND function is configured to a GPIO pin. The GPIO will output high when activated.
The option is selected between Indication when RF active and Indication when both MCU and RF active.;;help_end;;
